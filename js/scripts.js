    ...
    Plugin.prototype = {
        ...
        controlSlideShow: function(count){
            ...
            this.$document.on('keydown', function(e){
                ...
                // TODO keyboard navigation for slidejs and leaflet map
                else if (key == 40 && that.slideshow && !$('#slides').length) {
                    $('.icon-fullscreen').click();
                }
            });
            ...
            this.$control.on('click', function(){
                ...
                // TODO delete details if present
                deleteDetails();
            });
        ...
        // BUG!!! TODO change to scroll down to slideshow
        ...
        controlFullScreen: function(){
            ...
            this.$fsButton.css('display', 'inline-block').on('click', function(){
               ...
               // identify project index
               var i = that.current;
               // source correct img files
               for (var j = 0; j < IMAGES[i].details.length; j++) 
               { 
                    html += "<img src='img/" + IMAGES[i].details[j] + "'>";      
               }
               html += "</div>";
               // output html
               $('#slideshow').html(html);
               
               // Nathan Searles's slideshow plug-in
                $(function(){
                    $("#slides").slidesjs({
                        width: 1000,
                        height: 680,
               ...
               scrollToHere('#slideshow img', 1500);
               
               if (document.documentElement.clientWidth > 1024)
               {
                   // output map container
                   $('#footer').html("<div id='map'></div>");
                   
                   // load map
                   map();
                   // open to pop-up
                   IMAGES[that.current].marker.openPopup();
                   // add CSS shadow
                   $('#map').css("box-shadow", "3px 3px 6px #ccc");
               }
        ...
        quitSlideShow: function(test) {
            ...
            // TODO hide slideshow by deleting html
            $(function(){
                    deleteDetails(); 
            });
